...
function add_tinymce() {
  if (function_exists('wp_editor')) {} else { wp_tiny_mce( true, array( 'editor_selector' => 'tinyMCE-editor' ) ); }
}
...
global $themename, $shortname, $options;
  ...
  if (isset($_POST['action']) && 'save' == $_POST['action'] ) {
    ...
    foreach ($options as $value) {
      ...
      header('Location: themes.php?page=ace_options.php&saved=true');
      die;
    } else if(isset($_POST['action']) && 'reset' == $_POST['action'] ) {
    foreach ($options as $value) {
      delete_option( $value['id'] ); }
      header('Location: themes.php?page=ace_options.php&reset=true');
      die;
    }}
...
<link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/includes/style.css" type="text/css" media="screen" />
...
<div id="options-form">
  ...
  <div id="options-header">
    ...
    <div class="meta"><a href="https://twitter.com/hibluchic">Twitter</a> | <a href="http://www.facebook.com/hibluchic">Facebook</a></div>
  </div>
...
<?php } elseif ($value['type'] == 'image') { ?>
  ...
  <div class="wrap">
    ...
    <?php foreach ($value['options'] as $option) { ?>
      ...
      <img src="<?php bloginfo('template_url'); ?>/includes/images/<?php echo $option['value']; ?>" class="select-img" alt="<?php echo $option['name']; ?>" <?php if ( get_option( $value['id'] ) == $option['name']) { echo ' class="current"'; } elseif ($option == $value['std']) { echo ' class="current"'; } ?> /><br />
...
<?php } elseif ($value['type'] == 'color') { ?>
  ...
  <div class="wrap">
    <label for="<?php echo $value['id']; ?>"><h3><?php echo $value['name']; ?></h3></label>
    <label for="<?php echo $value['id']; ?>"><input type="text" class="<?php echo $value['id']; ?>" name="<?php echo $value['id']; ?>" value="<?php if ( get_option( $value['id'] ) != "") { echo get_option( $value['id'] ); } else { echo $value['std']; } ?>" /> <span class="<?php echo $value['id']; ?> button"><?php _e('Select','ace'); ?></span></label>
...
<!-- Screenshot -->
<?php } elseif ($value['type'] == 'screenshot') { ?><img src="<?php bloginfo('template_url'); ?>/includes/images/<?php echo $value['note']; ?>" class="screenshot" alt="" title="" />
